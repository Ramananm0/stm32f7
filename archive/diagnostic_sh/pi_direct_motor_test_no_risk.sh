...
echo "=== FORWARD DIRECT ==="
timeout 6s ros2 topic echo /wheel_velocity >/tmp/direct_wheel_velocity.txt 2>/dev/null &
E1=$!
ros2 topic pub -r 10 /cmd_vel geometry_msgs/msg/Twist "{linear: {x: 0.12, y: 0.0, z: 0.0}, angular: {x: 0.0, y: 0.0, z: 0.0}}" >/tmp/direct_cmd_vel_pub.txt 2>&1 &
P1=$!
sleep 3
...
tail -n 80 /tmp/direct_wheel_velocity.txt 2>/dev/null || true
