...
#define PID_MS       10u
#define PUB_IMU_MS   10u
#define PUB_ENC_MS   20u
#define LCD_MS      50u
...
static void update_safety(void)
{
    ...
    g_risk = S_LPF * g_risk + (1.0f - S_LPF) * r_raw;
...
static void motor_pid(float dt)
{
    ...
    for (uint8_t i = 0; i < MOTOR_NUM; i++) {
        ...
        command = target + KP * error + KI * g_integral[i];
        Motor_Set((Motor_ID)i, command);
    }
}

static void boot_test_apply(Motor_ID id, uint8_t forward)
{
    /* Match the known-good raw test style with explicit per-channel compares. */
    const uint32_t duty10 = 750u;
    const uint32_t duty11 = 750u;
    const uint32_t duty12 = 650u;
    const uint32_t duty13 = 800u;
    const uint32_t duty14 = 800u;
    const uint32_t duty2 = 800u;
    const uint32_t duty3 = 800u;

    Motor_StopAll();

    switch (id) {
    case MOTOR_FL:
        __HAL_TIM_SET_COMPARE(&htim10, TIM_CHANNEL_1, forward ? 0u : duty10);
        __HAL_TIM_SET_COMPARE(&htim11, TIM_CHANNEL_1, forward ? duty11 : 0u);
        break;
    case MOTOR_FR:
        __HAL_TIM_SET_COMPARE(&htim13, TIM_CHANNEL_1, forward ? 0u : duty13);
        __HAL_TIM_SET_COMPARE(&htim14, TIM_CHANNEL_1, forward ? duty14 : 0u);
        break;
    case MOTOR_RL:
        __HAL_TIM_SET_COMPARE(&htim12, TIM_CHANNEL_2, forward ? 0u : duty12);
        __HAL_TIM_SET_COMPARE(&htim12, TIM_CHANNEL_1, forward ? duty12 : 0u);
        break;
    case MOTOR_RR:
        __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_1, forward ? 0u : duty3);
        __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, forward ? duty2 : 0u);
        break;
    default:
        break;
    }
...
static void boot_test_clear_all_channels(void)
{
    ...
    __HAL_TIM_SET_COMPARE(&htim13, TIM_CHANNEL_1, 0u);
    __HAL_TIM_SET_COMPARE(&htim14, TIM_CHANNEL_1, 0u);
    __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, 0u);
    __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_1, 0u);
}

static void boot_test_apply_single(const char *label)
{
    boot_test_clear_all_channels();

    if (strcmp(label, "FL_A") == 0) {
        __HAL_TIM_SET_COMPARE(&htim10, TIM_CHANNEL_1, 750u);
    } else if (strcmp(label, "FL_B") == 0) {
        __HAL_TIM_SET_COMPARE(&htim11, TIM_CHANNEL_1, 750u);
    } else if (strcmp(label, "FR_A") == 0) {
        __HAL_TIM_SET_COMPARE(&htim13, TIM_CHANNEL_1, 800u);
    } else if (strcmp(label, "FR_B") == 0) {
        __HAL_TIM_SET_COMPARE(&htim14, TIM_CHANNEL_1, 800u);
    } else if (strcmp(label, "RL_A") == 0) {
        __HAL_TIM_SET_COMPARE(&htim12, TIM_CHANNEL_1, 650u);
    } else if (strcmp(label, "RL_B") == 0) {
        __HAL_TIM_SET_COMPARE(&htim12, TIM_CHANNEL_2, 650u);
    } else if (strcmp(label, "RR_A") == 0) {
        __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, 800u);
    } else if (strcmp(label, "RR_B") == 0) {
        __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_1, 800u);
    }
}
...
static void startup_motor_check(void)
{
    ...
    for (uint8_t i = 0; i < (sizeof(labels) / sizeof(labels[0])); i++) {
        LCD_Display_BootMotorTest(labels[i]);
        boot_test_apply_single(labels[i]);
        HAL_Delay(MOTOR_BOOT_TEST_RUN_MS);
        Motor_StopAll();
...
static void uros_setup(void)
{
    ...
    UROS_TRY_STAGE(216u, rclc_subscription_init_best_effort(&sub_cmd, &node,
    ...
    msg_status.data.size = 4;
    msg_status.data.capacity = 4;
...
void App_Run(void)
{
    ...
    LCD_Display_BootStatus(g_imu_ok, 0u, 0u);
